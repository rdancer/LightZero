...
def train_muzero_multi_task(
    ...
    policy = create_policy(cfgs[0].policy, model=model, enable_field=['learn', 'collect', 'eval'])
    if model_path is not None:
        policy.learn_mode.load_state_dict(torch.load(model_path, map_location=cfgs[0].policy.device))
    learner = BaseLearner(cfgs[0].policy.learn.learner, policy.learn_mode, tb_loggers[0], exp_name=cfgs[0].exp_name)
    ...
    for task_id, input_cfg in enumerate(input_cfg_list):
        ...
        tb_logger = SummaryWriter(os.path.join('./{}/log/'.format(cfg.exp_name), f'serial_task_{task_id}')) if get_rank() == 0 else None
